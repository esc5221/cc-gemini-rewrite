...
```
/rewrite-config policy lines    8줄 이상 답을 자동 재작성
/rewrite-config policy judge    LLM이 자동 재작성 여부 판정
/rewrite-config policy off      자동 재작성 안 함; /rewrite만 (기본값)
/rewrite-config off             플러그인 전체 끄기
