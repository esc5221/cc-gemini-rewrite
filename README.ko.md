...
Claude Code 답변이 어려우면, **turn이 끝나는 순간 LLM이 같은 내용을 이해되게 다시 써서 Claude Code 화면에 바로 붙인다.** 대화 루프·기록·API 히스토리는 건드리지 않는다.

```
당신:   이거 왜 느려? 원인이 뭐야
Claude: 여러 레이어가 얽혀서 상황에 따라 다를 수 있고, 캐시가 관여할 수도 있어
        경우에 따라 달라집니다. 여러 요인이 복합적이라 단정은 어렵습니다. …

        [cc-turn-ext] re-explained by Gemini        ← turn 끝에 자동으로 붙음
        원인은 캐시 미스다. 요청이 캐시를 못 타고 하위 계층까지 전부 내려가서 느려진다.
        확인: 캐시 히트율부터 봐라. 다음: 계층별 응답시간 로그로 병목 특정.
```
...
**답이 어려우면 `/rewrite`를 친다** — 바로 밑에 이해되는 버전이 스트리밍으로 뜬다. Claude를 안 거치고 Claude Code 안에 렌더된다. 아주 긴 답엔 자동으로도 뜨지만, 그건 가벼운 안전망이고 `/rewrite`가 메인이다.

### `/rewrite` — 메인

직전 답을 원할 때 재작성. **자동과 같은 주입 경로**로 렌더된다 (인라인 스트리밍, Claude 안 거침):

```
/rewrite
```

슬래시 커맨드는 일회성 플래그만 세우고, 그 turn 끝에 바이너리 주입이 발동해 마지막 실한 답을 강제 재작성한다 (슬래시 turn 자체의 응답은 건너뜀). 자동 정책이 `off`여도 동작.

스티어링도 된다 — 뒤에 요청을 붙이면 그 요청대로 재작성:
